#' ppmakDeriv
#'
#' Create the derivative of the Spline defined by given intervals and quadratic coefficients
...
ppmakDeriv = function(IntStarts, SpCoefs, Vectorised  = TRUE){
  if (!(Vectorised)){
    ...
    sp = function(PointToExamine){
      ...
      xmtMat = matrix(c(2*xmt,rep(1, Len), rep(0, Len)), ncol = 3, byrow = FALSE)
      Coefs = SpCoefs[IntervalNum,,drop = FALSE]
